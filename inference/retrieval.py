...
def prepare_song_database(csv_path: str, output_path: str) -> pd.DataFrame:
    """
    Prepare a song database from original CSV
    
    Args:
        csv_path: Path to the original CSV
        output_path: Path to save the prepared database
        
    Returns:
        DataFrame with prepared song data
    """
    # Load the CSV
    ...
    if isinstance(df['audio_embedding'].iloc[0], str):
        df['audio_embedding'] = df['audio_embedding'].apply(
    ...
    # Process each row to find unique songs
...
def build_song_database(model: torch.nn.Module, 
                        song_data_path: str, 
                        save_path: str, 
    ...
    with torch.no_grad():
        for i in range(0, len(song_dataset), batch_size):
            ...
            if 'link' in batch.columns:
                song_database['links'].extend(batch['link'].tolist())
            else:
                song_database['links'].extend([''] * len(batch))
...
def retrieve_songs_for_image(model: torch.nn.Module, 
    ...
    # Use model's similarity function if available, otherwise use dot product
    if hasattr(model, 'similarity'):
        similarities = model.similarity(image_embedding, song_embeddings).squeeze()
    else:
    ...
    for idx in top_indices:
        ...
        results.append({
            ...
            'link': song_database['links'][idx] if 'links' in song_database else ''
        })
